package net.darkness.mosaic.utils;


import net.darkness.mosaic.entity.ImageBlock;
...
public class ImageUtils {
    ...
    public static BlockResult convertToMosaica(InputStream originalStream, String size, String palette,
                                               Path convertedPath, Path visualPath) throws IOException {
        var originalImage = ImageIO.read(originalStream);
        var convertedImages = createMosaic(originalImage, size, palette);

        var convertedImage = convertedImages.first;
        var visualImage = convertedImages.second;

        ImageIO.write(convertedImage, "png", convertedPath.toFile());
        ImageIO.write(visualImage, "png", visualPath.toFile());

        System.out.println("Converted: " + convertedPath);
        System.out.println("Visual: " + visualPath);

        return getBlocksForMosaic(convertedImage, palette, size);
    }
    ...
    private static List<Integer> getColorsForBlock(BufferedImage image, int blockWidth, int blockHeight, int x, int y,
                                                   List<Color> palette) {
    ...
    // Return 1 if white
    // Else return number of color in palette, starting from 2
    ...
    private static Tuple2<BufferedImage, BufferedImage> createMosaic(BufferedImage originalImage, String size,
                                                                     String palette) {
        ...
        var convertedImage = new BufferedImage(desiredWidth, desiredHeight, BufferedImage.TYPE_INT_RGB);
        var visualImage = new BufferedImage(desiredWidth, desiredHeight, BufferedImage.TYPE_INT_RGB);

        var convertedGraphics = convertedImage.createGraphics();
        var visualGraphics = visualImage.createGraphics();

        convertedGraphics.setColor(Color.WHITE);
        visualGraphics.setColor(Color.WHITE);

        convertedGraphics.fillRect(0, 0, desiredWidth, desiredHeight);
        visualGraphics.fillRect(0, 0, desiredWidth, desiredHeight);
        ...
        var scaled = scaleImage(originalImage, width, height);

        convertedGraphics.drawImage(convertToMosaic(scaled, palette, MOSAIC_SIZE), x, y, width, height, null);
        visualGraphics.drawImage(scaled, x, y, width, height, null);

        convertedGraphics.dispose();
        visualGraphics.dispose();

        return Tuple2.of(convertedImage, visualImage);
    }
